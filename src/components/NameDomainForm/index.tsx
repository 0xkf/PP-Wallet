...
type NameDomainFormProp = {
  addresses: AddressArray;  // 送金先のアドレスを配列で受け取る
  twitterId: string;
  setStep: React.Dispatch<React.SetStateAction<number>>;
};

export const NameDomainForm: React.FC<NameDomainFormProp> = memo(
  ({ addresses, twitterId, setStep }) => {
        ...
        <div className="flex flex-col justify-center gap-y-6 items-center">
          <div className="flex flex-col items-center p-4 border-2 border-success rounded-md">
            {/* <div className="text-xl">
              address: <span className="text-success">{addresses}</span>
            </div> */}
            <div className="text-xl">
                address: <span className="text-success">{addresses.join(', ')}</span>
            </div>
            <div>
              <a
                href={`https://mumbai.polygonscan.com/address/${addresses}`}
                className="link"
              >
                Visit Polygon Scan
              </a>
